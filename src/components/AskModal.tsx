...
const AskModal = ({ askId, onReplySubmit }: any) => {
  const [replyText, setReplyText] = useState('');

  const handleReplySubmit = async () => {
    if (replyText) {
      const newAnswer = {title: replyText}; // Customize as needed
      await fetchFirestore.setDoc(`/asks/${askId}/answers`, newAnswer);
  ...
  return (
    ...
    <input
        ...
        className="w-full text-black px-4 py-2 rounded-l-md border border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
